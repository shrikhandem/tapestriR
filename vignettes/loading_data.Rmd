...
* h5 is the future format for miltiomics and multisample 
* loom is supported for DNA mutation only analysis
* Insights export is supported for DNA only analysis
...
# ideally would just start by loading filtered H5, but for now will load data with some basic filters
filtered_variants = filter_variants(experiment$assays$dna)
vaf=round(filtered_variants@data_layers$AD/filtered_variants@data_layers$DP, 3)
vaf[is.na(vaf)] <- 0
filtered_variants = add_data_layer(filtered_variants,'VAF',vaf)

#add the filtered data back to experiment, this will subset the rest of the assays to make sure we have same cells
...
In case you want to build a multiomics object from individual components, not using the `read_tap` function. 
