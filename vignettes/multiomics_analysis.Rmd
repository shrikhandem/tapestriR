...
Using the Mission Bio Tapestri Platform, reagents and the *tapestri* R package, we explore Single Nucleotide Variants (SNVs), protein expression and Copy Number Variations (CNVs), including loss of heterozygosity (LOH), which plays a large role in cancer evolution and contributes to cancer heterogeneity. More details can be found in the [Application Note] (https://missionbio.com/protein-reva-application-note-form/). 
...
We will start with the genotype data and explore also how we can identify clones, determine CNVs differences between clones. 
