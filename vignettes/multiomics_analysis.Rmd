...
res1<-glmpca(experiment$assays$dna$data_layers$VAF,L,fam="poi",verbose=TRUE)
projections[['Projection:glm_poi DR:glm_poi data:vaf']] = tibble(x = res1$loadings[,1], y = res1$loadings[,2])
res2<-glmpca(experiment$assays$dna$data_layers$VAF,L,fam="nb",verbose=TRUE)
projections[['Projection:glm_nb DR:glm_nb data:vaf']] = tibble(x = res2$loadings[,1], y = res2$loadings[,2])
res3<-prcomp(experiment$assays$dna$data_layers$VAF,center=TRUE,scale.=TRUE,rank.=L)
projections[['Projection:pca DR:pca data:vaf']] = tibble(x = res3$x[,1], y = res3$x[,2])

umap_values <- umap(experiment$assays$dna$data_layers$VAF, scale=TRUE, metric="euclidean", init="laplacian", pca=20) 
projections[['Projection:umap_euclidean DR:pca data:vaf']] = tibble(x = umap_values[,1], y = umap_values[,2])

umap_values <- umap(experiment$assays$dna$data_layers$VAF, scale=TRUE, metric="cosine", init="laplacian", pca=20) 
...
```{r fig.width = 12, fig.height = 8}
color_by = experiment$assays$dna$analysis_layers$umap_vaf_clusters %>% select(features.louvain.cluster,feature.kmean.cluster.4)
...
* row orders and clustering should be done outside of heatmap. heatmap is only for visualization 
*  
...
genotypes.mat = experiment$assays$dna@data_layers$NGT
cluster = experiment$assays$dna@analysis_layers$umap_vaf_clusters$umap.kmean.cluster.2
