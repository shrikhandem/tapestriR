...
moo = add_assay2(moo,filtered_variants)
moo = add_assay2(moo,cnv,keep_common_cells = TRUE)
moo = add_assay2(moo,protein, keep_common_cells = TRUE)
...
p = ggplot(experiment@dna@analysis_layers$umap_vaf)
...
cluster_on = experiment@dna@analysis_layers$umap_vaf
...
color_by = experiment@dna@data_layers$NGT %>% select(1:21) %>% mutate_all(as_factor) %>% mutate_all(recode_genotypes)
